...
class NotificationWorker(ctx: Context, params: WorkerParameters) : Worker(ctx, params) {
    ...
    private fun getPendingIntent(task: Task?): PendingIntent? {
        val intent = task?.let {
            Intent(applicationContext, DetailTaskActivity::class.java).apply {
    ...
    override fun doWork(): Result {
        ...
        if (flag) {
            val activeTask = runBlocking { taskRepository.getNearestActiveTask() }
                    ...
                    NotificationCompat.Builder(applicationContext, NOTIFICATION_CHANNEL_ID)
                        ...
                        .setContentText(
                            String.format(
                                ...
                                activeTask?.let { DateConverter.convertMillisToString(it.dueDateMillis) }
                            )
                        )
